...
public class TransactionDomainService {
    ...
    private final com.fiserv.uba.transaction.client.EntitlementClient entitlementClient;
    ...
    public TransactionDomainService(TellerTransactionRepository txRepository,
                                    ...
                                    com.fiserv.uba.transaction.client.EntitlementClient entitlementClient) {
    ...
    private void assertAllowed(String actorId, String operation, String branchId, BigDecimal amount) {
        ...
        try {
            decision = entitlementClient.check(actorId, operation, branchId, "UNKNOWN", amount);
        } catch (Exception ex) {
            throw new TransactionException(HttpStatus.SERVICE_UNAVAILABLE, "Entitlement policy service unavailable");
        }
        if (!decision.allowed()) {
            throw new TransactionException(HttpStatus.FORBIDDEN,
                    "Entitlement check failed: " + decision.reason());
        }
    }
